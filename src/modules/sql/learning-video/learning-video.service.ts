...
export class LearningVideoService extends ModelService<LearningVideo> {
  ...
  async createLearningVideoXls(job: Job): Promise<JobResponse> {
    try {
      ...
      const worksheet = workbook.addWorksheet('LearningVideo');
      ...
      const users: LearningVideo[] = JSON.parse(JSON.stringify(data));
      ...
      const folder = 'learning-video-excel';
