...
export class LearnYoutubeService extends ModelService<LearnYoutube> {
  ...
  async updateBulk(
    ...
    const productSort: LearnYoutube[] = [];
  ...
  async createLeanYutubeXls(job: Job): Promise<JobResponse> {
    try {
      ...
      const worksheet = workbook.addWorksheet('LearnYoututube');
      ...
      const users: LearnYoutube[] = JSON.parse(JSON.stringify(data));
