...
import { CreateUserExamsDto } from './dto/create-user-exams.dto';
import { UpdateUserExamsDto } from './dto/update-user-exams.dto';
import { UserExams } from './entities/user-exams.entity';
import { UserExamsService } from './user-exams.service';

const entity = snakeCase(UserExams.name);

@ApiTags(entity)
@ApiBearerAuth()
@ApiErrorResponses()
@ApiExtraModels(UserExams)
@Controller(entity)
export class UserExamsController {
  ...
  @ResponseUpdated(UserExams)
  async update(
    @Res() res: Response,
    @Owner() owner: OwnerDto,
    @Param('id') id: number,
    @Body() updateUserExamsDto: UpdateUserExamsDto,
  ) {
    const { error, data } = await this.userExamsService.update({
      owner,
      action: 'update',
      id: +id,
      body: updateUserExamsDto,
    });
  ...
  async findAll(
    ...
    const { error, data, offset, limit, count } =
      await this.userExamsService.findAll({
        owner,
        action: 'findAll',
        payload: { ...query },
      });
  ...
  async countAll(
    ...
    const { error, count } = await this.userExamsService.getCount({
      owner,
      action: 'getCount',
      payload: { ...query },
    });
  ...
  async delete(
    ...
    const { error, data } = await this.userExamsService.delete({
      owner,
      action: 'delete',
      id: +id,
      payload: { ...query },
